...
if file:
    ...
    ax.set_xlabel(selected_feature, color='white')
    ...
    if option == "Supervised Learning":
        ...
        if st.button("🔁 Run Regression"):
            ...
            ax.set_title("Actual vs Predicted Scatter Plot", color='white')
            ax.tick_params(axis='x', colors='white')
            ax.tick_params(axis='y', colors='white')
            fig.patch.set_alpha(0.0)
            ax.set_facecolor("none")
        ...
        if method == "KMeans Clustering":
            ...
            if 'PCA1' in clustered.columns and 'PCA2' in clustered.columns:
                ...
                ax.set_title("KMeans Clustering (PCA Projection)", color='white')
                ax.set_xlabel("PCA1", color='white')
                ax.set_ylabel("PCA2", color='white')
                ax.tick_params(axis='x', colors='white')
                ax.tick_params(axis='y', colors='white')
                fig.patch.set_alpha(0.0)
                ax.set_facecolor("none")
    ...
    plt.title("Hourly 5G Traffic Load", color='white')
    plt.xlabel("Hour of Day", color='white')
    plt.ylabel("Network Load (%)", color='white')
    ax.tick_params(axis='x', colors='white')
    ax.tick_params(axis='y', colors='white')
    fig.patch.set_alpha(0.0)
    ax.set_facecolor("none")
